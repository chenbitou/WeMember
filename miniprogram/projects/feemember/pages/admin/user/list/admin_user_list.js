...
Page({
	...
	bindPayTap: async function (e) {
		...
		let hint = type == 0 ? '未缴费' : '已缴费';

		let cb = async () => {
			try {
				await cloudHelper.callCloudSumbit('admin/user_pay', params).then(res => {

					let data1Name = 'dataList.list[' + idx + '].USER_PAY_TIME';
					let data2Name = 'dataList.list[' + idx + '].USER_PAY_STATUS';
					let data3Name = 'dataList.list[' + idx + '].USER_PAY_TYPE';
					this.setData({
						[data1Name]: timeHelper.time('Y-M-D h:m:s'),
						[data2Name]: type,
						[data3Name]: 1
					});
		...
		pageHelper.showConfirm('确认设置为' + hint + '?', cb);
	},
	...
	_getSearchMenu: async function () {
		...
		let sortMenus = [
			...
			{ label: '待注册', type: 'status', value: 7 },
			{ label: '禁用', type: 'status', value: 9 },
			{ label: '已缴费', type: 'pay', value: 1 },
			{ label: '未缴费', type: 'pay', value: 0 },
			{ label: '在线缴费', type: 'type', value: 0 },
			{ label: '后台缴费', type: 'type', value: 1 },
